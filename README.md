...
每天台灣時間 08:00 由 GitHub Actions 執行，關機後仍會運作。任務包含兩部分：
...
- `show_no_update`：選填，預設為 `true`；該來源當日 0 筆時仍會顯示「本次無新增項目。」。
...
所有一般來源當日 0 筆時，監控頁面都會保留該來源段落並顯示「本次無新增項目。」。

衛福部食藥署來源只查詢標題含「農藥殘留容許量標準」的公告，並只保留發布日期最新的一筆。最新版若發布於本次日期區間內，監控頁面在「衛福部食藥署」段落列出可點擊標題；不在區間內則顯示「本次無新增項目。」。目前已讀基準為 `id=31518`（2026-04-21），不會因新增監控設定而回填為當日更新。
...
這是每天 00:00 UTC；台灣全年 UTC+8，因此實際為 `Asia/Taipei` 每天 08:00。GitHub 伺服器負責執行，本機可關機。也可在 Actions 頁面手動 Run workflow，並選擇 dry-run。
...
一般來源與 ACRI 同步成功但沒有新項目時，監控頁面會在各自段落顯示「本次無新增項目。」。首次回填即使項目很多，也會完整列出所有本次新增標題。
