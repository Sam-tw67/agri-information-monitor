...
1. 擷取四個農業資訊來源前一週完整七個曆日的文章，建立或更新一筆「農業資訊監控」週報。
...
四個一般來源集中在 `sources.yml`，修改來源不需改 Python：
...
- `show_no_update`：選填，預設為 `true`；該來源本週 0 筆時仍會顯示「本次無新增項目。」。

農藥資訊服務網目前只收錄：公告修正農藥使用方法、公告農藥使用方法、預先通知公告農藥使用方法，以及標題以「更新通知」結尾的公告。代噴人員與空中施作代噴人員公告不會納入。

所有一般來源本週 0 筆時，週報都會保留該來源段落並顯示「本次無新增項目。」。

衛福部食藥署來源只查詢標題含「農藥殘留容許量標準」的公告，並只保留發布日期最新的一筆。最新版若發布於本次七日區間內，週報在「衛福部食藥署」段落列出可點擊標題；不在區間內則顯示「本次無新增項目。」。目前已讀基準為 `id=31518`（2026-04-21），不會因新增監控設定而回填為本週更新。
...
一般來源與 ACRI 同步成功但沒有新項目時，週報會在各自段落顯示「本次無新增項目。」。首次回填即使項目很多，也會完整列出所有本次新增標題。
